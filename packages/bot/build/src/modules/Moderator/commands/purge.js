...
const purge = new Command_1.default({
    name: 'purge',
    ...
    execute: function (ctx) {
        return purgeMessages(ctx, ctx.args.shift(), () => true, ctx.args.join(' '), 'purge');
    }
});
purge.command({
    name: 'user',
    ...
    execute: async function (ctx) {
        ...
        ctx.args.shift();
        return purgeMessages(ctx, ctx.args.shift(), msg => msg.author.id === user.id, ctx.args.join(' '));
    }
});
purge.command({
    name: 'embeds',
    ...
    execute: function (ctx) {
        return purgeMessages(ctx, ctx.args.shift(), msg => !!msg.embeds.length, ctx.args.join(' '));
    }
});
purge.command({
    name: 'bots',
    ...
    execute: function (ctx) {
        return purgeMessages(ctx, ctx.args.shift(), msg => msg.author.bot, ctx.args.join(' '));
    }
});
purge.command({
    name: 'humans',
...
purge.command({
    ...
    execute: function (ctx) {
        ...
        return purgeMessages(ctx, args.split(' ').shift(), msg => msg.content && msg.content.includes(text), reason);
    }
});
